...
// put the API into a function 
// going to add a transition animation for the scholarship matches

let scholarshipDatabase = []; // starts as an empty erray, later loaded with the parsed data from the JSON file
...
// start initalizing the country-picker API and take care of any bugs or errors that may take place
...
function initializeCountryPicker() {
  ...
  const codeInput = document.getElementById('countryCode');
    ...
    fetch(`https://api.countrylayer.com/v2/all?access_key=${encodeURIComponent(API_KEY)}`)
      ...
      .catch (err => {
        console.error("Could not fetch thecountry list", err.message);

      });

      input.addEventListener('input', () => {
        const val = input.value.trim();
        const match = Array.from(FileList.options).find(o => o.value === val);
